...
const CreateBatch: React.FC = () => {
  ...
  const handleEditName = async (id: string, name: string) => {
    ...
    try {
      const res = await axiosInstance.put("/api/admin/edit-batchname", {
        id,
        name,
      ...
      if (res.status === 200) {
        ...
        dispatch(
          updateBatchName({ id, name, batch_description: batch_description })
        );
        setEditBatch("");
  ...
  const handleCreateBatch = async () => {
    ...
    try {
      const res = await axiosInstance.post("/api/admin/new-batch", {
        name: newBatch,
      ...
      if (res.status === 201) {
        ...
        toast({
          ...
          description: "New batch has been added",
        });
      }
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col w-full mb-10 border p-6 rounded-md bg-white">
      ...
      <Tabs defaultValue="batch-create">
        <TabsList className="grid w-full grid-cols-2 gap-2 mb-4">
          ...
          <TabsTrigger value="batches" className="p-3 bg-gray-200 rounded-lg">
            All Batches
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="batches" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {batches.map((item: any) => (
              <Card
                key={item.id}
                className="p-4 rounded-md shadow-md space-y-4"
              >
                <div>
                ...
                <Input
                  placeholder="New batch description"
                ...
                <div className="flex space-x-2">
                  <Button
                    className="flex-1"
                    onClick={() => handleEditName(item.id, editBatch)}
                  >
                    Edit
                  </Button>
                  <Button
